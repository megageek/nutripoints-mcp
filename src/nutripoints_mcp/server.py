...
Create a new ingredient when necessary rather than forcing an unsuitable existing one. Before creating it, confirm
the user can obtain it; ask when availability is unclear. If required nutrition facts are missing, ask for them
...
serving_variant (for example, one egg) or a generic ingredient's base_servings. When creating a generic ingredient,
add its most useful base serving label and amount. A specific food can add multiple serving_variants; add sensible
ones when needed, such as pinch, teaspoon, and tablespoon for a spice. Use grams or milliliters when no suitable
named serving exists.
